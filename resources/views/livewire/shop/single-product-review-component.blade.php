<div class="blog-details content" x-data="{ showForm: @entangle('showForm') }">
    <div class="comments_area pb-5">
        <ul class="comment__list">
            ...
            @empty
            <a class="m-2">Jadilah yang Pertama Memberikan Ulasan!</a>
            @endforelse
        </ul>
    </div>
    @auth
    <div class="comment_respond" x-show="showForm">
        @if($canRate)
        @if($showForm)
        <h3 class="reply_title">{{ $currentRatingId ? 'Ulasanmu' : 'Tinggalkan Balasan' }}</h3>
        <form wire:submit.prevent="rate()" class="review__form score">
            ...
            <div class="input__box text-left">
                <textarea class="form-control" rows="5" wire:model.lazy="content">{{ old('Ulasan') }}</textarea>
                @error('content')<span class="text-danger">{{ $message }}</span>@enderror
            </div>
            <div class="submite__btn">
                @if($currentRatingId)
                @auth
                <button type="submit" class="btn btn-dark rounded shadow-lg">Perbarui</button>
                <button type="button" x-on:click="showForm = false" class="btn btn-secondary rounded shadow-lg">Tutup</button>
                @endauth
                @else
        ...
        <div class="alert alert-danger" role="alert">
            <small>Harus membeli produk terlebih dahulu sebelum memberikan ulasan</small>
        </div>
        @endif
    </div>
    @else
